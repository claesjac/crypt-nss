...
create_socket(pkg, type)
    ...
    CODE:
        ...
        if (strEQ(type, "tcp")) {
            socket->fd = PR_OpenTCPSocket(PR_AF_INET);
        }
...
set_option(self, option, value)
    ...
    CODE:
        if (SvPOK(option)) {
			...
			else if (strEQ(name, "ReuseAddr")) {
				sock_opt.option = PR_SockOpt_Reuseaddr;
				sock_opt.value.reuse_addr = value ? PR_FALSE : PR_TRUE;
			}
            else {
...
MODULE = Crypt::NSS     PACKAGE = Crypt::NSS::CertificateList

AV *
as_array(self)
    Crypt::NSS::CertificateList self;
    PREINIT:
        AV *certs;
    ...
    CODE:
        certs = newAV();
        for (i = 0, node = CERT_LIST_HEAD(self); !CERT_LIST_END(node, self); ++i, node = CERT_LIST_NEXT(node)) {
            cert = CERT_DupCertificate(node->cert);
        	cert_obj = newSV(0);
        	sv_setref_pv(cert_obj, "Crypt::NSS::Certificate", (void*) cert);
            av_push(certs, cert_obj);
        }
        RETVAL = certs;
    OUTPUT:
        RETVAL
...
get_nicknames(self)
    ...
    CODE:
        nicknames_arr = newAV();
        nicknames = CERT_NicknameStringsFromCertList(self, " [Expired]", "[Not valid]");
        for (i = 0; i < nicknames->numnicknames; i++) {
            fprintf(stderr, "%s\n", nicknames->nicknames[i]);
        }
        CERT_FreeNicknames(nicknames);
        RETVAL = nicknames_arr;
    OUTPUT:
        RETVAL
...
DESTROY(self)
    ...
    CODE:
        if (self != NULL) {
            CERT_DestroyCertList(self);
        }
        
MODULE = Crypt::NSS     PACKAGE = Crypt::NSS::Certificates

Crypt::NSS::CertificateList
find_user_certs_by_usage(pkg, usage, oneCertPerName=0, validOnly=0, pin_arg=&PL_sv_undef)
    const char * pkg;
    short   usage;
    ...
    CODE:
        certList = CERT_FindUserCertsByUsage(CERT_GetDefaultCertDB(), usage, oneCertPerName, validOnly, pin_arg);
        RETVAL = certList;
    OUTPUT:
        RETVAL
