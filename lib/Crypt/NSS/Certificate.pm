...
sub from_pem_file {
	...
	while (<$in>) {
		chomp;
		next unless /^(-----BEGIN CERTIFICATE-----)/ .. /^(-----END CERTIFICATE-----)/;
		next if $1;
		$data .= $_;
	}
	close($in);
	return $pkg->from_base64_DER($data);
}
...
=item from_pem_file ( $path : string ) : Crypt::NSS::Certificate

Loads the file at I<$path>, extracts the certificate if one is there and passes it off to
C<from_base64_DER>.
