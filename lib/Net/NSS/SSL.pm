...
sub new {
    ...
    my $is_server = $args{LocalAddr} && $args{LocalPort} ? 1 : 0;
    ...
    if ($is_server) {
        $sock->bind($args{LocalAddr}, $args{LocalPort});
		$sock->listen($args{Listen} || ());
    }

    # Upgrade to SSL socket
    $sock->import_into_ssl_layer();

    if (!$is_server && !exists $args{SSL_PKCS11_PinArg}) {
    ...
    # SSL Options
	$sock->set_option(SSL_SECURITY, SSL_OPTION_ENABLED);
    ...
    # Maybe connect
    if (!$is_server && !(exists $args{Connect} && !$args{Connect})) {
        if ($args{SSL_URL}) {
		...
		$sock->set_option(SSL_HANDSHAKE_AS_CLIENT, 1);
        
        $sock->connect($args{PeerHost}, $args{PeerPort}, ($args{Timeout} ? $args{Timeout} : ()));
    }
	elsif ($is_server) {		
		if (!exists $args{SSL_EnableSSLv3} || $args{SSL_EnableSSLv3}) {
			$sock->set_option(SSL_ENABLE_SSL3, SSL_OPTION_ENABLED);
		}
		
		$sock->set_option(SSL_ENABLE_TLS, $args{SSL_EnableTLS} ? SSL_OPTION_ENABLED : SSL_OPTION_DISABLED);
		$sock->set_option(SSL_ENABLE_SSL2, $args{SSL_EnableSSLv2} ? SSL_OPTION_ENABLED : SSL_OPTION_DISABLED);

		croak "Missing argument 'SSL_ServerCertNickname'" if !exists $args{SSL_ServerCertNickname};
		my $nickname = $args{SSL_ServerCertNickname};

		croak "Missing argument 'SSL_Password'" if !exists $args{SSL_Password};
		my $password = $args{SSL_Password};
		
		my $cert = Crypt::NSS::PKCS11->find_cert_by_nickname($nickname, $password);
		my $key = Crypt::NSS::PKCS11->find_key_by_any_cert($cert, $password);
		
		$sock->configure_as_server($cert, $key);				
	}
...
package Net::NSS::SSL::AsGlob;

use Carp qw(croak);

sub TIEHANDLE {
    my ($pkg, $socket) = @_;
    my $self = bless [$socket], $pkg;
    return $self;
}
...
sub PRINT {
    my $self = shift;
    my $written = 0;
    $written += $self->[0]->syswrite($_) for @_;
    return $written;
}

sub PRINTF {
    my $self = shift;
    return $self->[0]->syswrite(sprintf(@_));
}

sub READ {
    my $self = shift;
    return $self->[0]->sysread(@_);
}
...
sub DESTROY {
    my $self = shift;
    if (ref $self eq "ARRAY" && defined $self->[0]) {
        $self->[0]->close();
    }
}
...
It's possible to rewrite a glob version of your socket that can be used as a standard filehandle.

=over 4

=item as_glob ( $pkg )

Returns a glob version of the socket tied to the class I<$pkg> or C<Net::NSS::SSL::AsGlob> if ommitted.
